...
//! CLI subcommand for VKD scaling benchmarks.
//!
...
//! ```
//!
//! ## Adding a new VKD implementation
//!
//! 1. Create a new file (e.g. `my_vkd_setup.rs`) implementing
//!    `vkd::bench::VkdBenchmarkSetup` and [`cache::BenchCache`] for your
//!    directory type.
//! 2. Add `mod my_vkd_setup;` in this file.
//! 3. Add a match arm in `run()` for `--impl my-vkd`.
//!
//! Then run:
//! ```text
//! cargo run -p examples --release -- scaling-bench --impl my-vkd --sizes 10,14
//! ```
...
pub(crate) struct Args {
    ...
    /// VKD implementation to benchmark (e.g. "akd")
...
async fn run_with<S: BenchCache>(args: &Args, format: Format, sizes: &[usize], ops: &[Op]) {
    ...
    match format {
        Format::Table => {
            println!("VKD Scaling Benchmarks: {}", S::name());
