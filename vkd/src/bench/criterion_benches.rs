...
//! Criterion benchmark functions for VKD implementations.

use super::VkdBenchmarkSetup;
use crate::traits::VerifiableKeyDirectory;
use criterion::{BatchSize, Criterion};

/// Register publish benchmarks.
pub fn bench_publish<S: VkdBenchmarkSetup>(c: &mut Criterion) {
    ...
    let id = format!("VKD publish ({} entries) [{}]", num_entries, S::name());
...
pub fn bench_lookup<S: VkdBenchmarkSetup>(c: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .unwrap();
    ...
    let num_entries = 1000;
    let id = format!("VKD lookup ({} entries) [{}]", num_entries, S::name());
    c.bench_function(&id, |b| {
        b.iter_batched(
            || {
                let dir = runtime.block_on(S::create_directory());
...
where
    S: VkdBenchmarkSetup,
    F: Fn(&<S::Directory as VerifiableKeyDirectory>::LookupProof) -> usize,
{
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
    ...
    c.bench_function(&id, |b| {
        b.iter_batched(
            ...
            |(pk, epoch_hash, label, proof)| {
                <S::Directory as VerifiableKeyDirectory>::lookup_verify(
                    &pk,
                    epoch_hash.hash(),
                    epoch_hash.epoch(),
                    label,
...
pub fn bench_key_history<S: VkdBenchmarkSetup>(c: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .unwrap();
    ...
    let num_epochs = 5;
    let id = format!(
        "VKD key_history ({} entries, {} epochs) [{}]",
        num_entries,
        num_epochs,
        S::name()
    );
...
pub fn bench_audit<S: VkdBenchmarkSetup>(c: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .unwrap();
    ...
    let num_entries = 1000;
    let id = format!("VKD audit ({} entries) [{}]", num_entries, S::name());
    c.bench_function(&id, |b| {
        b.iter_batched(
            || {
                let dir = runtime.block_on(S::create_directory());
...
pub fn bench_audit_verify<S: VkdBenchmarkSetup>(c: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .unwrap();
    ...
    c.bench_function(&id, |b| {
        b.iter_batched(
            ...
            |(hashes, proof)| {
                runtime
                    .block_on(<S::Directory as VerifiableKeyDirectory>::audit_verify(
                        hashes, proof,
                    ))
                    .unwrap();
            },
